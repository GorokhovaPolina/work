...
def main():
    ...
    # Ищем все JSON файлы в новой структуре папок
    json_files = sorted(glob.glob("../res_Nastya_upd_dataset/runlist_item_0000/clip_*/snapshot_0000.json"))
    if not json_files:
        print("ОШИБКА: Нет JSON файлов в структуре ../res_Nastya_upd_dataset/runlist_item_0000/clip_*/")
        return
    ...
    for json_path in json_files:
        ...
        runlist_num = path_parts[-3]
        clip_num = path_parts[-2]
        filename = f"{runlist_num}_{clip_num}"
        ...
        # === ВИЗУАЛИЗАЦИЯ ===
        img_relative_path = json_path.replace('snapshot_0000.json', 'snapshot_0000.jpg')
        img_path = img_relative_path
        
        # Альтернативный вариант поиска изображения
        if not os.path.exists(img_path):
            # Пробуем найти в папке с изображениями
            img_dir = os.path.dirname(json_path)
            img_files = glob.glob(os.path.join(img_dir, "*.jpg"))
            if img_files:
                img_path = img_files[0]  # Берем первый JPG файл
