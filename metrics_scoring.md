...
| Диапазон | Смысл |
|---|---|
| `(0.85, 1.0]` | Уверенное выполнение (Accept — зеленый) |
| `(0.75, 0.85]` | Хорошее выполнение (Accept — светло-зеленый) |
| `(0.65, 0.75]` | Выше среднего (Wait — светло-зеленый) |
| `(0.50, 0.65]` | Среднее (Wait — желтый) |
...
`max_deviation` — максимальное угловое отклонение от стартовой позы в правильном направлении, накопленное за все время инструкции.
...
```
ortho_velocity_penalty = clamp(integral(ratio^2·dt) / T, 0, 1)
ratio = clamp(v_ortho / ortho_norm, 0, 1),  ortho_norm = 5°/с
```

Квадратичная зависимость: малые дрожания (v_ortho ≈ 1°/с) дают ratio^2 ≈ 0.04, большие движения (v_ortho = 5°/с) дают ratio^2 = 1. Интеграл нормируется на суммарное время T → результат в [0, 1].
...
Кадр правильный: `|yaw| + |pitch| <= 7°` (нейтральная зона). Счет идет по одному условию — двойного инкремента нет.
...
`stable_time` растет при `score > 0.65`, не изменяется при `(0.50, 0.65]`, обнуляется при `score <= 0.50`.
